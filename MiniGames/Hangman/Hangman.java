...
public class Hangman {
	...
	public boolean targetContains(String target, char letter) {
		...
		int count = 0;
		// Compares each character in target string to determine if letter is in word
		while (count < target.length())
		{
			if (letter == target.charAt(count))
			{
				isinword = true;
				break;
			}
	...
	public boolean hasFoundTarget(String target, String guess) {
		...
		// Checks for a match
		boolean match = target.toLowerCase().equals(guess.toLowerCase());
	...
	public String getCurrentGuessForDisplay(String currentGuess) {
		...
		// Replaces _ with *
		currentGuess = currentGuess.replace('*', '_');
		String display = "";

		// Adds a space after each char
		for(int count=0; count<currentGuess.length()-1; ++count)
			display += currentGuess.charAt(count) + " ";

		// Adds final char
		display += currentGuess.charAt(currentGuess.length()-1);
	...
	public String updateCurrentGuess(String target, String guess, char letter) {
		...
		// Replaces correct characters
		String updatedguess = "";
		for (int count = 0; count<target.length(); ++count)
			if (target.charAt(count) == letter)
				updatedguess += letter;
			else
				updatedguess += guess.charAt(count);
